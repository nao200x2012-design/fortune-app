...
# 必須JSONスキーマ（このキーのみ）
{
  ...
  "overview": "総合の流れと運勢の波。**九星と十二運星のデータが、どのようにユーザーの今日の運勢と願いに影響しているかを詳細に分析し、**読者が納得する具体的な根拠を提示せよ。**目標文字数は${overviewLength}文字**。**最低でも5段落**に分けて記述すること。",
...
# 特別・追加指示
- 「overview」の本文に、**九星（${kyuusei}）**と**十二運星（${juuniunsei}）**の言葉を必ず盛り込み、その性質が現在の運勢にどう作用しているかを説明せよ。
- 「advice」は必ず**${adviceCount}件**。
- **${sexualBlock}**
- **誘惑の秘術モードでない場合**は、**「advice」の2件目以降（仕事運、金運、健康運、精神面）**は、**関西弁の女親分**の口調を維持せよ。
...
`.trim();
    ...
    res.json({ ok:true, data: { ...data, biorhythm: biorhythmData, isSexualActive, sexualRankings }, raw:txt }); 
  }catch(e){
