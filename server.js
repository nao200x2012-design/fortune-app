...
import { v4 as uuidv4 } from 'uuid'; // ユーザーID生成のためのライブラリ
import cookieParser from 'cookie-parser'; // Cookieを扱うためのライブラリ

dotenv.config();

const app = express();
// Cookieの利用とCORS設定
app.use(cors({ origin: true, credentials: true })); // credentials: true を追加
app.use(express.json());
app.use(cookieParser()); // Cookie Parserを有効化
...
// ユーザーごとの利用状況を記憶するインメモリデータベース
// 形式: { userId: [{ timestamp: 1700000000000 }, { timestamp: ... }, ...] }
const usageStore = {}; 
...
function rateLimitMiddleware(req, res, next) {
    let userId = req.cookies.fortuneUserId;

    // 1. ユーザーIDの確認と設定
    if (!userId) {
        userId = uuidv4();
        // CookieにユーザーIDを設定（有効期限: 1年）
        res.cookie('fortuneUserId', userId, { maxAge: 365 * 24 * ONE_HOUR_MS, httpOnly: true, secure: true, sameSite: 'Lax' });
    }
    ...
    // 3. 利用状況のクリーンアップとカウント
    const userRequests = usageStore[userId] || [];
    
    // 1時間以上前のリクエストを削除してメモリを節約
    const recentRequests = userRequests.filter(req => req.timestamp > oneHourAgo);

    // 4. 制限チェック
    if (recentRequests.length >= MAX_REQUESTS_PER_HOUR) {
        // 制限を超えている場合はエラーを返す
        const nextRequestTime = new Date(recentRequests[0].timestamp + ONE_HOUR_MS);
        const waitTimeMinutes = Math.ceil((nextRequestTime.getTime() - now) / 60000);
        
        // 利用回数を最新の状態に更新してからエラーレスポンスを返す
        usageStore[userId] = recentRequests;
        
        return res.status(429).json({
            ok: false,
            error: `利用制限: 1時間あたり${MAX_REQUESTS_PER_HOUR}回までです。あと${waitTimeMinutes}分後に再度お試しください。`
        });
    }
...
app.post("/fortune", rateLimitMiddleware, async (req,res)=>{
  try{
...
`.trim();
    ...
    // クライアントに返信する際に、現在の利用回数も渡す
    const currentUsageCount = usageStore[req.cookies.fortuneUserId] ? usageStore[req.cookies.fortuneUserId].length : 0;
    
    res.json({ ok:true, data: { ...data, biorhythm: biorhythmData, isSexualActive, sexualRankings, isSingleMethod, currentUsageCount } , raw:txt }); 
  }catch(e){
    console.error(e); 
    // 429エラーの場合は、クライアントにエラーメッセージをそのまま渡す
    if (e.message && e.message.includes("利用制限:")) {
...
// Note: 'uuid'と'cookie-parser'を使うには、npm installが必要です。
